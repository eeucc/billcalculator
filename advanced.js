...
document
  ...
  .addEventListener("click", function calculateBill() {
    ...
    if (!currentReading || !startDate || !endDate) {
      resultText.textContent = "Please fill in all fields.";
      return;
    }

    //count days in each month
    function calculateDaysPerMonth(startDate, endDate) {
      ...
      for (
        ...
        if (!daysPerMonth[monthYearKey]) {
          daysPerMonth[monthYearKey] = 0;
        }

        // Increment the count for the month
        daysPerMonth[monthYearKey]++;
      }
